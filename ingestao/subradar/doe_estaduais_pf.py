"""
Conector: DOE Estaduais — Diários Oficiais de SP, MG e RJ

Complementa o DOU federal (DO1/DO2/DO3) com publicações em diários estaduais.
Relevante para: servidores públicos estaduais/municipais, fornecedores estaduais,
gestores de contratos estaduais e munícipes de SP/MG/RJ.

Portais e APIs:
  SP  — Imprensa Oficial SP: pesquisa.doe.sp.gov.br (busca fulltext)
  MG  — IOIMPRENSA MG: pesquisa.iof.mg.gov.br (busca fulltext)
  RJ  — IOERJ: https://www.ioerj.com.br/portal/ (portal simples)

Classificação: mesma lógica de palavras-chave do DOU PF (dou_pf.py).
Usa Haiku se ANTHROPIC_API_KEY estiver disponível.
...
_CRITICO_KW = {
    "demitido", "demitida", "exonerado por justa causa", "preso", "presa",
    "condenado", "condenada", "indiciado", "indiciada", "cassado", "cassada",
    "processo disciplinar", "sindicância", "demissão a bem do serviço",
    "descredenciado", "descredenciada", "suspensão de habilitação",
}
_ATENCAO_KW = {
    "penalidade", "advertência", "suspensão", "multa", "notificado", "notificada",
    "autuado", "autuada", "impedimento", "irregularidade", "glosa", "infração",
    "processo administrativo", "inquérito administrativo", "sindicância",
    "rescisão contratual", "descredenciamento", "habilitação cancelada",
}

# Estado SP: API oficial requer credenciamento gov.
#   Scraping do portal público doe.sp.gov.br (busca avançada).
# Estado MG: DSpace REST no acervo legado (até 2023) + scraping do portal novo.
# Estado RJ: POST form em busca_do.php — retorna HTML.
...
_ESTADOS = [
    ...
    },
    {
        "sigla": "MG",
        "nome": "Minas Gerais",
        # DSpace REST — acervo histórico sem autenticação
        "method": "GET",
    ...
    },
    {
        ...
        # POST form field: "Palavra-chave", "Tipo" (Normal), "Ordem" (data_desc)
        "params_fn": lambda nome, desde: {
            "Palavra-chave": nome,
            "Tipo": "Normal",
            "Ordem": "data_desc",
...
def _nome_presente(nome: str, texto: str) -> bool:
    ...
    presentes = sum(1 for t in tokens if len(t) > 3 and t in texto_n)
    return presentes >= 2
...
def _buscar_doe_estado(estado: dict, nome: str, dias: int = 90) -> list[dict]:
    ...
    try:
        ...
        # Fallback: extrai menções do HTML (sem BeautifulSoup)
        html = resp.text
        return _parse_doe_html(html, nome, estado["sigla"])
...
def _parse_doe_html(html: str, nome: str, sigla: str) -> list[dict]:
    """Extrai resultados básicos do HTML de busca DOE."""
    resultados = []
    # Captura blocos de resultado com título e extrato
    blocos = re.findall(
        r"(?:class=['\"](?:resultado|ato|item|entry)['\"][^>]*>)(.*?)(?=class=['\"](?:resultado|ato|item|entry)['\"]|$)",
        html, re.DOTALL | re.IGNORECASE
    )
    if not blocos:
        # Tenta extrair qualquer parágrafo que contenha o nome
        paragrafos = re.findall(r"<p[^>]*>(.*?)</p>", html, re.DOTALL | re.IGNORECASE)
        blocos = [p for p in paragrafos if _nome_presente(nome, re.sub(r"<[^>]+>", " ", p))]

    for bloco in blocos[:5]:
        texto_limpo = re.sub(r"<[^>]+>", " ", bloco).strip()
        texto_limpo = re.sub(r"\s+", " ", texto_limpo)
        if _nome_presente(nome, texto_limpo):
            resultados.append({
                "estado": sigla,
...
def _classificar_doe(titulo: str, texto: str, nome: str, sigla: str) -> dict:
    """Classifica publicação via Haiku ou keywords."""
    if ANTHROPIC_KEY:
        prompt = (
            f"Classifique esta publicação do Diário Oficial de {sigla} para '{nome}'.\n"
            f"Texto: {titulo[:200]} — {texto[:300]}\n"
            "Responda em JSON: "
            '{"adverso": true/false, "severidade": "critico"/"atencao"/"nenhum", "motivo": "10 palavras"}\n'
            "critico=demissão/processo criminal/cassação, atencao=penalidade/suspensão/irregularidade, nenhum=nomeação/contrato normal"
        )
        try:
            resp = requests.post(
                _HAIKU_URL,
...
class DOEEstaduaisPFConnector(SubradarSource):
    """
    Busca menções do nome nos Diários Oficiais de SP, MG e RJ.
    Classifica publicações adversas com Haiku (fallback: keywords).
    ...
    def consultar_cnpj(self, cnpj_or_cpf: str, razao_social: str | None = None, **_) -> list[dict]:
        ...
        if not nome or len(nome.split()) < 2:
            logger.debug("doe_estaduais_pf: nome ausente ou incompleto — pulando")
            return []
        ...
        for estado in _ESTADOS:
            publicacoes = _buscar_doe_estado(estado, nome)
            for pub in publicacoes:
                ...
                if not _nome_presente(nome, f"{titulo} {texto}"):
                    continue

                classif = _classificar_doe(titulo, texto, nome, estado["sigla"])
                ...
                logger.info("doe_estaduais_pf: publicação [%s] no DOE-%s para '%s'",
                            severidade, sigla, nome)
