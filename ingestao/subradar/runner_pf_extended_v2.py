"""
Subradar PF — Extended V2 (Consumer + 4 Novas Fontes)
...
logger = logging.getLogger("subradar.runner_pf_extended_v2")

# ---------------------------------------------------------------------------
# Pipeline PF Extended V2 — 20 + 4 = 24 fontes
# ---------------------------------------------------------------------------

FONTES_PF_EXTENDED_V2 = FONTES_PF_EXTENDED + [
    CartorioImoveisCofiexConnector(),  # Cartório — Imóveis registrados
]
...
def processar_cpf(
    ...
    ciclo = _ciclo_atual()
    fontes = FONTES_PF_EXTENDED_V2

    logger.info("Subradar PF Extended V2 — %s | %d fontes | dry_run=%s",
    ...
    if dry_run:
        ...
        for a in todos_alertas[:15]:
            sev = a.get("severidade", "?").upper()
            print(f"  [{sev}] {a.get('titulo', '')}")
        if len(todos_alertas) > 15:
            print(f"  ... e mais {len(todos_alertas) - 15}")
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Subradar PF Extended V2 — 24 fontes")
